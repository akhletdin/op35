...
/* Many-to-one связь - добавление новой колонки в таблицу сотрудников - city_id с внешним ключом на таблицу cities */
/* Например, если у нас есть таблица сотрудников и таблица городов,
то у каждого сотрудника может быть только один город,
но у каждого города может быть много сотрудников */
...
/* One-to-one связь - выборка сотрудников с информацией о паспортных данных */
/* Например, если у нас есть таблица сотрудников и таблица с паспортными данными,
то у каждого сотрудника может быть только один паспорт,
но у каждого паспорта может быть только один сотрудник */


/* Выборка сотрудников с информацией о паспортных данных */
SELECT e.full_name,
       pi.passport_number,
       pi.issue_date,
       pi.issue_place
  FROM employees AS e
       INNER JOIN
       passport_info AS pi ON e.id = pi.employee_id;

/* Создание представления (VIEW) на основе выборки сотрудников с информацией о паспортных данных */
CREATE VIEW employees_passport_info AS
    SELECT e.full_name,
           pi.passport_number,
           pi.issue_date,
           pi.issue_place
...
/* Many-to-many связь - создание новой таблицы с любимыми фильмами сотрудников */
/* Например, если у нас есть таблица сотрудников и таблица с фильмами,
то у каждого сотрудника может быть много любимых фильмов,
но у каждого фильма может быть много любящих сотрудников */
